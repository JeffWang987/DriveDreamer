...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    r"""A conditional 2D UNet model that takes a noisy sample, conditional
    ...
    def __init__(
        ...
        if addition_embed_type == 'text':
            ...
            # here as this is exactly the required dimension for the currently only use
    ...
    def unload_lora(self):
        ...
        deprecate(
            'unload_lora',
            '0.28.0',
            'Calling `unload_lora()` is deprecated and will be removed in a future version. Please install `peft` '
            'and then call `disable_adapters().',
        )
        for module in self.modules():
        ...
        if self.config.addition_embed_type == 'text':
            ...
            if 'image_embeds' not in added_cond_kwargs:
                raise ValueError(
                    ...
                    f'the keyword argument `image_embeds` to be passed in `added_cond_kwargs`'
                )
            ...
            if 'text_embeds' not in added_cond_kwargs:
                raise ValueError(
                    ...
                    f'the keyword argument `text_embeds` to be passed in `added_cond_kwargs`'
                )
            text_embeds = added_cond_kwargs.get('text_embeds')
            if 'time_ids' not in added_cond_kwargs:
                raise ValueError(
                    f"{self.__class__} has the config param `addition_embed_type` set to 'text_time' which requires "
                    f'the keyword argument `time_ids` to be passed in `added_cond_kwargs`'
                )
            ...
            if 'image_embeds' not in added_cond_kwargs:
                raise ValueError(
                    f"{self.__class__} has the config param `addition_embed_type` set to 'image' which requires "
                    f'the keyword argument `image_embeds` to be passed in `added_cond_kwargs`'
                )
            image_embs = added_cond_kwargs.get('image_embeds')
            ...
            if 'image_embeds' not in added_cond_kwargs or 'hint' not in added_cond_kwargs:
                raise ValueError(
                    ...
                    f'the keyword arguments `image_embeds` and `hint` to be passed in `added_cond_kwargs`'
                )
            image_embs = added_cond_kwargs.get('image_embeds')
